    ...
    <div className="fixed inset-0 z-50 flex flex-col" style={{background:'#0a1520'}}>
      ...
      <div className="flex items-center justify-between px-5 py-3 border-b shrink-0"
        ...
        <button onClick={onClose} className="text-2xl leading-none"
                ...
                onMouseLeave={e=>e.target.style.color='#5f7080'}>✕</button>
      </div>
              ...
              {destData && (
                <div className="rounded-xl p-4 space-y-3" style={{background:'rgba(201,168,76,0.06)', border:'1px solid rgba(201,168,76,0.25)'}}>
                  ...
                  {destData.BUDGET_NOTE && (
                    <p className="text-sm leading-relaxed" style={{color:'#a8b8c8'}}>💰 {destData.BUDGET_NOTE}</p>
                  )}
                  {destData.WEATHER && (
                    <p className="text-sm leading-relaxed" style={{color:'#a8b8c8'}}>🌤️ {destData.WEATHER}</p>
                  )}
                  ...
                  {destData.DISH && (
                    <p className="text-xs" style={{color:'#5f7080'}}>🍴 Must try: <span style={{color:'#8a9bb0'}}>{destData.DISH}</span></p>
                  )}
                </div>
              )}
                ...
                <div className="space-y-2">
                  ...
                  {flightDetails?.outboundDate && (
                    <div className="flex justify-between text-sm">
                      ...
                      <span style={{color:'#a8b8c8'}}>{fmtDate(flightDetails.outboundDate)} · arrive {flightDetails.arrivalTime}</span>
                    </div>
                  )}
                  {flightDetails?.returnDate && (
                    <div className="flex justify-between text-sm">
                      <span style={{color:'#5f7080'}}>Return</span>
                      <span style={{color:'#a8b8c8'}}>{fmtDate(flightDetails.returnDate)} · depart {flightDetails.departureTime}</span>
                    </div>
                  )}
                  {selectedHotel && (
                    <div className="flex justify-between text-sm">
                  ...
                  {prefs?.groupType && (
                    <div className="flex justify-between text-sm">
                      ...
                      <span style={{color:'#a8b8c8'}}>{prefs.groupType} · {prefs.numAdults} adults</span>
                    </div>
                  )}
                </div>
              </div>

              {/* Cost guide — formatted */}
            ...
            <div className="space-y-4">
              ...
              {skyscannerUrl && (
                <div className="rounded-xl p-4" style={{background:'#1a2d3e', border:'1px solid rgba(201,168,76,0.2)'}}>
                  <p className="text-xs font-bold uppercase tracking-widest mb-1" style={{color:'#c9a84c'}}>✈️ Flights</p>
              ...
              {bookingUrl && (
                <div className="rounded-xl p-4" style={{background:'#1a2d3e', border:'1px solid rgba(201,168,76,0.2)'}}>
                  <p className="text-xs font-bold uppercase tracking-widest mb-1" style={{color:'#c9a84c'}}>🏨 Hotels</p>
                  <p className="text-xs mb-3" style={{color:'#5f7080'}}>
              ...
              {carHireUrl && (
                <div className="rounded-xl p-4" style={{background:'#1a2d3e', border:'1px solid rgba(201,168,76,0.2)'}}>
                  <p className="text-xs font-bold uppercase tracking-widest mb-1" style={{color:'#c9a84c'}}>🚗 Car Hire</p>
              ...
              <div className="rounded-xl overflow-hidden" style={{border:'1px solid rgba(201,168,76,0.2)'}}>
                <div className="px-4 py-2.5" style={{background:'rgba(201,168,76,0.12)'}}>
                  <p className="text-xs font-bold uppercase tracking-widest" style={{color:'#c9a84c'}}>🎟️ Activities & Experiences</p>
                </div>
